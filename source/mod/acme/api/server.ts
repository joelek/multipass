	...
	endpoints.push((raw, auxillary) => {
		...
		return {
			...
			validateRequest: async () => {
				...
				let guard = autoguard.api.wrapMessageGuard(shared.Autoguard.Requests["downloadCertificate"], serverOptions?.debugMode);
				let request = guard.as({ options, headers, payload }, "request");
				return {
					handleRequest: async () => {
						let response = await routes["downloadCertificate"](new autoguard.api.ClientRequest(request, false, auxillary));
						return {
							validateResponse: async () => {
								let guard = autoguard.api.wrapMessageGuard(shared.Autoguard.Responses["downloadCertificate"], serverOptions?.debugMode);
